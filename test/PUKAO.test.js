...
const PUKAO = artifacts.require('PUKAO');

contract('PUKAO', function ([_, creator, investor]) {
  beforeEach(async function () {
    this.token = await PUKAO.new({ from: creator });
  });

  it('has a name', async function () {
    (await this.token.name()).should.equal('PUKAO');
  });

  it('has a symbol', async function () {
    (await this.token.symbol()).should.equal('PKO');
  });
